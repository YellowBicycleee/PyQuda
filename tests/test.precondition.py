...
def test_precondition():
  ...
  U1_without_eo = U1.lexico()

  U2 = gauge_utils.unitGauge(latt_size, Nc)
  U3 = gauge_utils.unitGauge(latt_size, Nc)
...
#   print(f'U1_without_eo[0, 0, 0, 0, 0, 0] = {U1_without_eo.shape}')
#   print(f'difference between U2 and U1_without_eo: {cp.linalg.norm(U1_without_eo - np.array(U2.data)) / cp.linalg.norm(U1_without_eo)}')
  print(f'difference between U1 and U3: {cp.linalg.norm(U1.data - U3.data) / cp.linalg.norm(U2.data)}')

  # 对U2奇偶预处理
  qcu.gauge_eo_precondition(U3.data_ptr, U2.data_ptr, double_prec)

  print(f'U1[0, 1, 0, 0, 0, 0]    = {U1.data[0, 1, 0, 0, 0, 0]}')
  print(f'U2[0, 0, 0, 0, 0, 1]    = {U2.data[0, 0, 0, 0, 0, 1]}')

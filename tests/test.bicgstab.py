...
def test_mpi(my_m_input, warm_flag = False):
  ...
  r0 = b_mrhs[0].data[1] + 0.125 * b.data[1]
  b.data[0][:] = r0
  ...
  # r0 = new_b 
  # print(f'new b norm = {cp.linalg.norm(r0_norm)}')
  # print(f'new b = {r0[0,0,0,0]}')
  for i in range (max_iteration) :
    r1_r0_prod = cp.dot(r1.reshape(1, -1).conj(), r0.reshape(-1, 1))
    b.data[0][:] = p
    ...
    Ap = copy.copy(b.data[1])
    Ap_r0_prod = cp.dot(Ap.reshape(1, -1).conj(), r0.reshape(-1, 1))
    alpha = r1_r0_prod / Ap_r0_prod
    s_j = r1 - alpha * Ap

    # As_j = s_j kappa^2 D_{oe} D_{eo} s_j
    b.data[0][:] = s_j
    quda.dslashQuda(temp.even_ptr, b.even_ptr, quda_dslash.invert_param, QudaParity.QUDA_EVEN_PARITY)
    quda.dslashQuda(b.odd_ptr, temp.even_ptr, quda_dslash.invert_param, QudaParity.QUDA_ODD_PARITY)
    b.data[1][:] = b.data[0] - 0.125 * 0.125 * b.data[1]
    As_j = copy.copy(b.data[1])
  
    As_j_s_j_prod = cp.dot(As_j.reshape(1, -1).conj(), s_j.reshape(-1, 1))
    As_j_As_j_prod = cp.dot(As_j.reshape(1, -1).conj(), As_j.reshape(-1, 1))
    omega = As_j_s_j_prod / As_j_As_j_prod
    ...
    if (cp.linalg.norm(r1) / r0_norm < max_prec):
      ...
      break
    r_1_r0_prod = cp.dot(r1.reshape(1, -1).conj(), r0.reshape(-1, 1))
    beta = alpha / omega * r_1_r0_prod / r1_r0_prod
    ppp = p - omega * Ap
    p_new = r1 + beta * ppp
